...
//Will take max of 100 points/vertices as input
struct vertices
{
	int x;
	int y;
...
edge create_edge(struct vertices v1, struct vertices v2) {
	...
	edge node;
	//For determining the min of y coordinate
	if(y1>y2)
	{
		node.Y_max = y1;
		node.X_min = x2;
...
//Sorting with respect to xmin inside each bucket
void bsort(vector<edge> &aet) {
	int n = aet.size();
	edge temp;
	if (n == 0) 
		return;
...
void display(void) {
	...
	//scanline starting from ymin till ymax
	for (int i = global_y_min; i <= global_y_max; i++) {
		for (int j = 0; j < aet.size(); j++) {
			if (aet[j].Y_max <= i) {
				...
				//updated aet after removal of aet[j]
				aet = temp;
				j--;
		...
		if (aet.size() >=2) {
			for (int l = 0; l < aet.size() - 1; l+=2) {
				...
				//pairing the points from aet and displaying
				ptx1 = aet[l].X_min;
				ptx2 = aet[l+1].X_min;
				pty1 = i;
				pty2 = i;				
...
int main(int argc, char** argv){
	...
	//to complete the polygon, edge is created between first vertex and the last vertex 
